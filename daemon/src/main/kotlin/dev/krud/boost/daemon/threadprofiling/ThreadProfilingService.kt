...
class ThreadProfilingService(
    ...
    fun runProfiling() {
        ...
        val expired = runningThreadProfiling.filter { it.finishTime.time < System.currentTimeMillis() }.map { it.id }
        expired.forEach { updateProfilingRequestToFinished(it) }

        val groupedByInstance = runningThreadProfiling.filter { !expired.contains(it.id) }.groupBy { it.instanceId }

        groupedByInstance.forEach { (instanceId, requests) ->
            val threadDumpRequest = actuatorClientProvider.provide(instanceId).threadDump()
            val threadDump = threadDumpRequest.getOrNull()
            if(threadDumpRequest.isSuccess && threadDump != null) {
                requests.forEach { request ->
                    val threads = threadDump.threads.filter { thread -> thread.stackTrace.find { it.className == request.className } != null }
                    threads.forEach { thread ->
                        val threadLog = ThreadProfilingLog(request.id)
                        //todo put thread data
    ...
    fun getLogsForRequest(requestId: UUID): List<ThreadProfilingLog> {
        ...
        return crudHandler.index(where {
