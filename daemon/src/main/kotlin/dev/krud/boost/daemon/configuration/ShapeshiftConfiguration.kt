...
class ShapeshiftConfiguration : ShapeShiftBuilderCustomizer, ShapeShiftAutoConfiguration() {
    override fun customize(builder: ShapeShiftBuilder) {
        builder
            ...
            .withObjectSupplier { EventLogRO(UUID.randomUUID(), LocalDateTime.now(), EventLogType.INSTANCE_HEALTH_CHANGED, EventLogSeverity.INFO, UUID.randomUUID()) }
            .withObjectSupplier { InstanceHealthLogRO(LocalDateTime.now(), UUID.randomUUID()) }
    }
}