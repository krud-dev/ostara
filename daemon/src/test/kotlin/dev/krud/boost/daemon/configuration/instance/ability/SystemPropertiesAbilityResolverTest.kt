...
class SystemPropertiesAbilityResolverTest {
    private val resolver = SystemPropertiesAbilityResolver()
    ...
    fun `resolver should return true if has property source`() {
        ...
        whenever(actuatorClient.env()).thenReturn(
            Result.success(
                EnvActuatorResponse(
                    emptySet(),
                    listOf(EnvActuatorResponse.PropertySource("systemProperties", emptyMap()))
                )
            )
        )
